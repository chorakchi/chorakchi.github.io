(function () {
    "use strict";
    angular
        .module('historicalAllocationFa', [])
        .config(routeconfig);

    function routeconfig($stateProvider) {
        $stateProvider.state('panel_fa.historicalAllocation', {
            url: '/historicalAllocation',
            templateUrl: "asset/app/component/panel/historicalAllocation/historicalAllocationFa.tpl.html"
        });
    }
    angular
        .module('historicalAllocationFa')
    ...
    angular
        .module('historicalAllocationFa')
        .controller('historicalAllocationFaCtrl', function ($scope, $state, loader, $timeout) {
            $scope.ManagementTypeCode = '0';
            $scope.historicalAllocationReportFrom = moment(new Date()).add(-10, 'days').format('jYYYY/jMM/jDD');
            $scope.historicalAllocationReportTo = moment(new Date()).add(-1, 'days').format('jYYYY/jMM/jDD');
            ...
            $scope.historicalAllocationReportGenerate = function () {
                ...
                $.ajax({
                    ...
                    data: {
                        "ReportFilter": {
                            "DateFilter": {
                                "StartDate": moment($scope.historicalAllocationReportFrom,'jYYYY/jMM/jDD').format('YYYY-MM-DD'),
                                "EndDate": moment($scope.historicalAllocationReportTo, 'jYYYY/jMM/jDD').format('YYYY-MM-DD')
                            },
                    ...
                    success: function (result) {
                        ...
                        for (var prop3 = 0 ; prop3 <= 1 ; prop3++) {
                            ...
                            for (var prop1 in tempFromOrTo) {
                                ...
                                assetAllocationObj = {
                                    'name': tempFromOrTo[prop1].AssetAllocation.AssetTitleFa,
                                    'y': tempFromOrTo[prop1].AssetAllocation.NetValue,
                                    'drilldown': tempFromOrTo[prop1].AssetAllocation.AssetTitleFa,
                                ...
                                for (var prop2 in tempFromOrTo[prop1].AssetAllocations) {
                                    ...
                                    datatempObj = {
                                        'name': tempFromOrTo[prop1].AssetAllocations[prop2].AssetTitleFa,
                                ...
                                seriesTempObj = {
                                    'name': tempFromOrTo[prop1].AssetAllocation.AssetTitleFa,
                                    'id': tempFromOrTo[prop1].AssetAllocation.AssetTitleFa,
                                    'data': datatempArr
                                };
                        ...
                        for (var elementarrayselector in tempAssetAllocationArr) {
                            for (var assetAllocationArrcount in tempAssetAllocationArr[elementarrayselector]) {
                                if (tempAssetAllocationArr[elementarrayselector][assetAllocationArrcount].name === "نقد و معادل وجه نقد" && tempAssetAllocationArr[elementarrayselector][assetAllocationArrcount].y < 0) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        for (var prop1 in result.Result) {
                            ...
                            categories.push(moment(result.Result[prop1].Date.split("T")[0],'YYYY-M-D').format('jYYYY/jM/jD'));
